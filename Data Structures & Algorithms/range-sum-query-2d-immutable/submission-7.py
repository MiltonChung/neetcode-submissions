    ...
    def __init__(self, matrix: List[List[int]]):
        ...
        for row in range(len(matrix)):
            for col in range(len(matrix[row])):
                top = self.prefix[row - 1][col] if row - 1 >= 0 else 0
                left = self.prefix[row][col - 1] if col - 1 >= 0 else 0
                topLeft = self.prefix[row - 1][col - 1] if col - 1 >= 0 and row - 1 >= 0 else 0
                self.prefix[row][col] = top + left + matrix[row][col] - topLeft
